...
#              built like the harness: pinned nightly, --cfg=pulley_tail_calls,
...
#
# TOOLS="wasmtime zwasm wamr" (default: all) selects what to (re)build.
set -euo pipefail
ROOT="$(cd "$(dirname "$0")/.." && pwd)"
OUT="${ROOT}/target/cm-tools"
mkdir -p "${OUT}"
TOOLS=" ${TOOLS:-wasmtime zwasm wamr} "
NIGHTLY_TC="$(grep -m1 '^NIGHTLY_TC=' "${ROOT}/scripts/build-lib.sh" | cut -d'"' -f2)"
TMP="$(mktemp -d)"
trap 'rm -rf "${TMP}"; git -C "${ROOT}/zwasm" worktree prune; git -C "${ROOT}/wasm-micro-runtime" worktree prune' EXIT

if [[ "${TOOLS}" == *" wasmtime "* ]]; then
echo "==> wasmtime CLI (Pulley + component model async)"
# Its own target dir, so these RUSTFLAGS don't invalidate other builds.
( cd "${ROOT}/wasmtime"
  PATH="${HOME}/.rustup/toolchains/${NIGHTLY_TC}-aarch64-apple-darwin/bin:${PATH}" \
  RUSTFLAGS="--cfg=pulley_tail_calls -C target-cpu=apple-a12" \
    cargo build --release -p wasmtime-cli --no-default-features \
      --target-dir "${ROOT}/target/cm-tools-build" \
      --features run,cranelift,pulley,component-model,component-model-async,gc,gc-drc,wat,parallel-compilation )
cp "${ROOT}/target/cm-tools-build/release/wasmtime" "${OUT}/wasmtime"
fi
